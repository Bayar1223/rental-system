...
function MyApplications() {
  ...
  const handleCancel = async (id) => {
    ...
    try {
      ...
      setApps((prev) =>
        prev.map((a) => (a._id === id ? { ...a, status: "cancelled", ...(res.data || {}) } : a))
      );
    } catch (err) {
    ...
    <div className="min-h-screen pt-20" style={{ background: "#0A0A0A", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <header className="max-w-6xl mx-auto px-6 lg:px-12 py-12">
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-[10px] tracking-[0.3em] uppercase" style={{ color: "#C9A84C" }}>Applications</span>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 lg:px-12 mb-10">
        <div className="flex flex-wrap gap-1 overflow-x-auto" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          {FILTER_TABS.map((tab) => (
            <button
              key={tab.v} onClick={() => setFilter(tab.v)}
        ...
        {loading ? <LoadingList />
          ...
          : (
            <div className="space-y-5">
              {filtered.map((app) => (
                <ApplicationCard key={app._id} app={app} onCancel={handleCancel} actioning={actioningId === app._id} />
              ))}
            </div>
          )
        }
...
function ApplicationCard({ app, onCancel, actioning }) {
  ...
  // ⭐ ЗАСВАР: server-ийн талбарын нэрс — images, monthlyRent, location.*
    ...
    >
      ...
      <div className="md:col-span-6 flex flex-col">
        ...
        {(startDateStr || leaseMonths) && (
          <div className="flex flex-wrap gap-4 mb-3 text-xs text-white/60">
            {startDateStr && (
              <span><span className="text-white/40">Эхлэх: </span>{startDateStr}</span>
            )}
            {leaseMonths && (
              <span><span className="text-white/40">Хугацаа: </span>{leaseMonths} сар</span>
            )}
          </div>
        )}

        {app.message && (
          <div className="mt-2 p-3 text-xs text-white/60 leading-relaxed"
      ...
      {[...Array(3)].map((_, i) => (
        ...
        >
          ...
          <div className="md:col-span-6 space-y-3">
            ...
            <div className="h-3 w-full mt-4" style={{ background: "rgba(255,255,255,0.04)" }} />
          </div>
          <div className="md:col-span-3 space-y-2">
            <div className="h-9 w-full" style={{ background: "rgba(255,255,255,0.05)" }} />
            <div className="h-9 w-full" style={{ background: "rgba(201,168,76,0.08)" }} />
          </div>
        </div>
      ))}
    </div>
...
function EmptyState({ filter }) {
  const messages = {
    all:       { title: "Өргөдөл байхгүй",            body: "Та хараахан байр түрээслэх хүсэлт явуулаагүй байна." },
    pending:   { title: "Хүлээгдэж буй өргөдөл байхгүй", body: "Шинэ өргөдлийн хариу хүлээгдэж байна." },
    approved:  { title: "Зөвшөөрсөн өргөдөл байхгүй", body: "Танай өргөдлүүд зөвшөөрөгдөхөд эндээс харагдана." },
    rejected:  { title: "Татгалзсан өргөдөл байхгүй", body: "Сайн байна! Татгалзсан өргөдөл одоогоор алга." },
    cancelled: { title: "Цуцалсан өргөдөл байхгүй",   body: "Та хараахан өргөдөл цуцлаагүй байна." },
  };
