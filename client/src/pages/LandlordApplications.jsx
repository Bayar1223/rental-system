...
function LandlordApplications() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setError(err.response?.data?.message || "Өргөдөл татаж чадсангүй");
      } finally {
  ...
  const handleApprove = async (id) => {
    if (!confirm("Та энэ өргөдлийг зөвшөөрөхдөө итгэлтэй байна уу? Энэ нь гэрээ үүсгэх процесс эхлүүлнэ.")) return;
    setActioningId(id);
    try {
      const res = await api.put(`/api/applications/${id}/status`, { status: "approved" });
      setApps((prev) =>
        prev.map((a) => (a._id === id ? { ...a, status: "approved", ...(res.data || {}) } : a))
      );
    } catch (err) {
  ...
  const handleReject = async (id) => {
    if (!confirm("Та энэ өргөдлийг татгалзахдаа итгэлтэй байна уу?")) return;
    setActioningId(id);
    try {
      const res = await api.put(`/api/applications/${id}/status`, { status: "rejected" });
      setApps((prev) =>
        prev.map((a) => (a._id === id ? { ...a, status: "rejected", ...(res.data || {}) } : a))
      );
    } catch (err) {
    ...
    <div className="min-h-screen pt-20" style={{ background: "#0A0A0A", fontFamily: "'DM Sans', sans-serif" }}>
      <header className="max-w-7xl mx-auto px-6 lg:px-12 py-12">
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-[10px] tracking-[0.3em] uppercase" style={{ color: "#C9A84C" }}>Incoming Applications</span>
        </div>
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          ...
          >
            Ирсэн<br />
            <em style={{ color: "#C9A84C", fontStyle: "italic" }}>өргөдлүүд</em>
          </h1>
          ...
          {counts.pending > 0 && (
            ...
            >
              <div className="text-[10px] tracking-[0.25em] uppercase text-amber-300/70 mb-1">Шинэ хүлээгдэж буй</div>
              <div className="font-light"
                style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, color: "#F59E0B" }}
              >{counts.pending}</div>
            </div>
          )}
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-12 mb-10">
        <div className="flex flex-wrap gap-1 overflow-x-auto mb-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          {FILTER_TABS.map((tab) => (
        ...
        {properties.length > 1 && (
          <div className="flex items-center gap-4">
            <span className="text-[10px] tracking-[0.25em] uppercase text-white/40 whitespace-nowrap">Байр:</span>
            ...
            >
              <option value="" style={{ background: "#141414" }}>Бүх байр ({properties.length})</option>
              {properties.map((p) => (
                <option key={p._id} value={p._id} style={{ background: "#141414" }}>{p.title}</option>
              ))}
            </select>
            {propertyFilter && (
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-12 pb-20">
        ...
        {loading ? <LoadingList />
          : filtered.length === 0 ? <EmptyState filter={filter} hasProperty={!!propertyFilter} />
          : (
            <div className="space-y-5">
              {filtered.map((app) => (
                <LandlordApplicationCard key={app._id} app={app}
...
function LandlordApplicationCard({ app, onApprove, onReject, actioning }) {
  ...
  // ⭐ ЗАСВАР: images, monthlyRent, location.*
  ...
  const date = app.createdAt
    ? new Date(app.createdAt).toLocaleDateString("mn-MN", { year: "numeric", month: "long", day: "numeric" })
    : "";

  // ⭐ Lease info
  const startDateStr = app.startDate
    ? new Date(app.startDate).toLocaleDateString("mn-MN", { year: "numeric", month: "short", day: "numeric" })
    : null;
  const leaseMonths = app.leaseMonths;
  ...
  return (
    <article className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-5 transition-all duration-300"
    ...
    >
      ...
      <div className="lg:col-span-6">
        ...
        {/* ⭐ Lease info */}
        {(startDateStr || leaseMonths) && (
          <div className="flex flex-wrap gap-4 mb-5 text-xs text-white/60">
            {startDateStr && <span><span className="text-white/40">Эхлэх: </span>{startDateStr}</span>}
            {leaseMonths  && <span><span className="text-white/40">Хугацаа: </span>{leaseMonths} сар</span>}
          </div>
        )}

        {/* Tenant info — virtual name талбар */}
        <div className="pt-4 mb-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="text-[9px] tracking-[0.3em] uppercase text-white/40 mb-3">Түрээслэгч</div>
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm text-white mb-1">{tenant.name || "Хэрэглэгч"}</div>
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-white/55">
                {tenant.email && (
                  <a href={`mailto:${tenant.email}`} className="flex items-center gap-1.5 hover:text-white transition-colors">
                    <span style={{ color: "#C9A84C" }}>◇</span>{tenant.email}
                  </a>
                )}
                {tenant.phone && (
                  <a href={`tel:${tenant.phone}`} className="flex items-center gap-1.5 hover:text-white transition-colors">
                    <span style={{ color: "#C9A84C" }}>◇</span>{tenant.phone}
                  </a>
                )}
              </div>
            </div>
        ...
        {app.message && (
          <div className="p-3 text-xs text-white/60 leading-relaxed"
            style={{ background: "rgba(255,255,255,0.02)", borderLeft: "1px solid rgba(201,168,76,0.3)" }}
          >
            <div className="text-[9px] tracking-[0.25em] uppercase text-white/40 mb-1.5">Захидал</div>
            {app.message}
          </div>
        )}
      </div>
      ...
      <div className="lg:col-span-4 flex flex-col gap-3 lg:justify-between">
        <div className="flex flex-wrap gap-2">
          ...
          {app.status === "approved" && app.contractStatus && (
            <StatusPill label={`Гэрээ: ${contractInfo.label}`} color={contractInfo.color} />
          )}
        </div>

        <div className="flex flex-col gap-2 mt-auto">
          {canApprove && (
            <button onClick={() => onApprove(app._id)} disabled={actioning}
              ...
              onMouseEnter={(e) => !actioning && (e.currentTarget.style.background = "#E8D49E")}
              onMouseLeave={(e) => !actioning && (e.currentTarget.style.background = "#C9A84C")}
            >
              {actioning ? "..." : "Зөвшөөрөх"}
              {!actioning && <span className="transition-transform duration-300 group-hover:translate-x-1">→</span>}
            </button>
          )}
          {canReject && (
            <button onClick={() => onReject(app._id)} disabled={actioning}
            ...
            >{actioning ? "..." : "Татгалзах"}</button>
          )}
          {canViewContract && (
            <Link to={`/contract/${app._id}`}
...
function LoadingList() {
  return (
    <div className="space-y-5">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-5 animate-pulse"
          style={{ background: "#141414", border: "1px solid rgba(201,168,76,0.08)" }}
        >
          <div className="lg:col-span-2 aspect-[4/3]" style={{ background: "rgba(255,255,255,0.03)" }} />
          <div className="lg:col-span-6 space-y-3">
            ...
            <div className="h-4 w-32" style={{ background: "rgba(201,168,76,0.1)" }} />
          </div>
          <div className="lg:col-span-4 space-y-2">
            <div className="h-9 w-full" style={{ background: "rgba(201,168,76,0.1)" }} />
...
function EmptyState({ filter, hasProperty }) {
  const messages = {
    all: hasProperty
      ? { title: "Энэ байранд өргөдөл алга", body: "Сонгосон байранд хараахан өргөдөл ирээгүй байна." }
      : { title: "Өргөдөл байхгүй",         body: "Танай байруудад хараахан түрээслэгчийн өргөдөл ирээгүй байна." },
    pending:   { title: "Хүлээгдэж буй өргөдөл алга", body: "Бүх өргөдлийн хариу өгсөн байна." },
    approved:  { title: "Зөвшөөрсөн өргөдөл алга",    body: "Та хараахан өргөдөл зөвшөөрөөгүй байна." },
    rejected:  { title: "Татгалзсан өргөдөл алга",    body: "Та хараахан өргөдөл татгалзаагүй байна." },
    cancelled: { title: "Цуцалсан өргөдөл алга",      body: "Цуцалсан өргөдөл одоогоор байхгүй." },
  };
