...
const ACTIVE_CONTRACT_STATUSES = ["pending_signatures", "signed", "payment_pending", "active"];
...
function MyRentals() {
  ...
  // ⭐ ЗАСВАР: monthlyRent (price биш)
  const totalMonthly = useMemo(() => {
    return rentals
      .filter((r) => r.contractStatus === "active")
      .reduce((sum, r) => sum + (r.property?.monthlyRent || r.property?.price || 0), 0);
  }, [rentals]);
    ...
    <div className="min-h-screen pt-20" style={{ background: "#0A0A0A", fontFamily: "'DM Sans', sans-serif" }}>
      <header className="max-w-6xl mx-auto px-6 lg:px-12 py-12">
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-[10px] tracking-[0.3em] uppercase" style={{ color: "#C9A84C" }}>Active Rentals</span>
        </div>
        ...
        >
          Миний<br />
          <em style={{ color: "#C9A84C", fontStyle: "italic" }}>түрээсүүд</em>
        </h1>
      </header>

      {!loading && rentals.length > 0 && (
        <section className="max-w-6xl mx-auto px-6 lg:px-12 mb-10">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3" style={{ border: "1px solid rgba(201,168,76,0.15)" }}>
            <div className="p-6" style={{ borderRight: "1px solid rgba(201,168,76,0.08)" }}>
              <div className="text-[10px] tracking-[0.25em] uppercase text-white/40 mb-2">Нийт түрээс</div>
              <div className="font-light leading-none"
                style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 40, color: "#C9A84C" }}
              >{rentals.length}</div>
            </div>
            <div className="p-6" style={{ borderRight: "1px solid rgba(201,168,76,0.08)" }}>
              <div className="text-[10px] tracking-[0.25em] uppercase text-white/40 mb-2">Идэвхтэй</div>
              <div className="font-light leading-none"
                style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 40, color: "#10B981" }}
              >{counts.active || 0}</div>
            </div>
            <div className="p-6 col-span-2 md:col-span-1">
              <div className="text-[10px] tracking-[0.25em] uppercase text-white/40 mb-2">Сарын нийт зардал</div>
              <div className="font-light leading-none"
                style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, color: "#C9A84C" }}
              >{new Intl.NumberFormat("mn-MN").format(totalMonthly)}₮</div>
            </div>
          </div>
        </section>
      )}
      ...
      <main className="max-w-6xl mx-auto px-6 lg:px-12 pb-20">
        ...
        {loading ? <LoadingList />
          : filtered.length === 0 ? <EmptyState hasRentals={rentals.length > 0} />
          : (
            <div className="space-y-5">
              {filtered.map((rental) => <RentalCard key={rental._id} rental={rental} />)}
            </div>
          )
        }
      </main>
...
function RentalCard({ rental }) {
  ...
  const contractInfo = CONTRACT_STATUS[rental.contractStatus] || CONTRACT_STATUS.pending_signatures;
  ...
  const date = rental.updatedAt
    ? new Date(rental.updatedAt).toLocaleDateString("mn-MN", { year: "numeric", month: "long", day: "numeric" })
    : "";
  ...
  return (
    <article className="grid grid-cols-1 md:grid-cols-12 gap-6 p-5 transition-all duration-300"
    ...
    >
      ...
      <div className="md:col-span-6">
        ...
        <p className="text-xs text-white/50 mb-3">{address}</p>
        ...
        {/* ⭐ Landlord — virtual name талбар хэрэглэнэ */}
        {landlord.name && (
          <div className="pt-4 flex items-center gap-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            >{landlord.name.charAt(0).toUpperCase()}</div>
            <div className="flex-1 min-w-0">
              <div className="text-[9px] tracking-[0.3em] uppercase text-white/40 mb-0.5">Байрны эзэн</div>
...
function StatusPill({ label, color }) {
  return (
    <div className="inline-flex items-center gap-2 px-3 py-1.5 text-[10px] tracking-[0.2em] uppercase self-start"
...
function LoadingList() {
  return (
    <div className="space-y-5">
      {[...Array(2)].map((_, i) => (
        <div key={i} className="grid grid-cols-1 md:grid-cols-12 gap-6 p-5 animate-pulse"
          style={{ background: "#141414", border: "1px solid rgba(201,168,76,0.08)" }}
        >
          <div className="md:col-span-3 aspect-[4/3]" style={{ background: "rgba(255,255,255,0.03)" }} />
...
function EmptyState({ hasRentals }) {
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-white/50 max-w-md mb-8 leading-relaxed">
          Та хараахан байр түрээслээгүй байна. Цуглуулгаас сонирхсон байраа сонгож өргөдөл гаргаарай.
        </p>
        <Link to="/home"
  ...
  return (
    ...
    >
      <p className="text-sm text-white/50">Сонгосон төлөвт түрээс байхгүй</p>
    </div>
  );
}
